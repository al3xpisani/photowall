...
var mercury_group,
  ...
  sun_group,
  skybox,
  orbit_outlines; // 3D objects and groups. Hierarchy is (in descending order of importance) orbit_group > planet_group. Sun and skybox group are special exceptions.
...
var options = new (function () {
  ...
  this.Render_Updated_Scaling = function () {
    UpdateScene();
  };
...
function init() {
  ...
  renderer = new THREE.WebGLRenderer({
    ...
    alpha: true,
  }); // Logarithmic depth buffer set to true causes severe shader artifacts.
  ...
  var Camera_Focus = datGUI.add(options, "CameraFocus", [
    "Sun",
    "Mercury",
    "Venus",
    "Earth",
  ...
  OrbitalFolder.add(options, "PlanetScale", 1, 300);
  ...
  SunEffectsFolder.add(options, "sun_effect_noise", 0.0, 1.0);
  SunEffectsFolder.add(options, "sun_effect_speed", 0.0, 1.0);
  ...
  }); // THis never worked. //datGUI.close(); //Add our 3D scene to the html web page!
  ...
  lights[1] = new THREE.PointLight(0xffffff, 22, 1000000, 2);
  ...
  this.sun_flare = new THREE.LensFlare(
    lensflare0,
    200,
    0.0,
...
function TraceOrbitOutlines() {
  orbit_outlines.add(
    CreateOrbitalLine(
      Mercury.semimajor_axis_scene(),
      Mercury.semiminor_axis_scene(),
      Mercury.periapsis_scene(),
      Mercury.orbital_inclination,
      Mercury.longitude_ascending,
      Mercury.argument_periapsis,
      Mercury.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Venus.semimajor_axis_scene(),
      Venus.semiminor_axis_scene(),
      Venus.periapsis_scene(),
      Venus.orbital_inclination,
      Venus.longitude_ascending,
      Venus.argument_periapsis,
      Venus.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Earth.semimajor_axis_scene(),
      Earth.semiminor_axis_scene(),
      Earth.periapsis_scene(),
      Earth.orbital_inclination,
      Earth.longitude_ascending,
      Earth.argument_periapsis,
      Earth.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Mars.semimajor_axis_scene(),
      Mars.semiminor_axis_scene(),
      Mars.periapsis_scene(),
      Mars.orbital_inclination,
      Mars.longitude_ascending,
      Mars.argument_periapsis,
      Mars.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Jupiter.semimajor_axis_scene(),
      Jupiter.semiminor_axis_scene(),
      Jupiter.periapsis_scene(),
      Jupiter.orbital_inclination,
      Jupiter.longitude_ascending,
      Jupiter.argument_periapsis,
      Jupiter.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Saturn.semimajor_axis_scene(),
      Saturn.semiminor_axis_scene(),
      Saturn.periapsis_scene(),
      Saturn.orbital_inclination,
      Saturn.longitude_ascending,
      Saturn.argument_periapsis,
      Saturn.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Uranus.semimajor_axis_scene(),
      Uranus.semiminor_axis_scene(),
      Uranus.periapsis_scene(),
      Uranus.orbital_inclination,
      Uranus.longitude_ascending,
      Uranus.argument_periapsis,
      Uranus.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Neptune.semimajor_axis_scene(),
      Neptune.semiminor_axis_scene(),
      Neptune.periapsis_scene(),
      Neptune.orbital_inclination,
      Neptune.longitude_ascending,
      Neptune.argument_periapsis,
      Neptune.orbital_eccentricity
    )
  );
  orbit_outlines.add(
    CreateOrbitalLine(
      Pluto.semimajor_axis_scene(),
      Pluto.semiminor_axis_scene(),
      Pluto.periapsis_scene(),
      Pluto.orbital_inclination,
      Pluto.longitude_ascending,
      Pluto.argument_periapsis,
      Pluto.orbital_eccentricity
    )
  );
...
function CreateOrbitalLine(
  ...
  for (var i = 0; i < 2 * Math.PI + 0.02; i = i + 0.01) {
    ...
    var y =
      R * Math.sin(i + argument_periapsis) * Math.sin(orbital_inclination);
    var x =
      R *
...
function CreateSpriteText(text, colour, name, offset) {
  var SpriteText = new THREE_Text.SpriteText2D(text, {
...
function CalculateDistanceFromObject(
  camera_x,
  camera_y,
  camera_z,
  object_x,
...
function ScaleOverlaySpheres(
  ...
  var distance_from_planet = CalculateDistanceFromObject(
    camera.position.x,
    camera.position.y,
    camera.position.z,
    distance_from_group.position.x,
    distance_from_group.position.y,
    distance_from_group.position.z
  );
  object_group.getObjectByName(sphere_name, true).scale.x =
    distance_from_planet / scale_constant;
  object_group.getObjectByName(sphere_name, true).scale.y =
    distance_from_planet / scale_constant;
  object_group.getObjectByName(sphere_name, true).scale.z =
    distance_from_planet / scale_constant;
}

function ScalePlanet(planet, scale_constant) {
  planet.parent_group.getObjectByName(
    planet.name,
    true
  ).scale.x = scale_constant;
  planet.parent_group.getObjectByName(
    planet.name,
    true
  ).scale.y = scale_constant;
  planet.parent_group.getObjectByName(
    planet.name,
    true
  ).scale.z = scale_constant;
}
...
function AdjustPlanetLocation(group, planet) {
  ...
  console.log("before", group.position);
  var R =
  ...
  group.position.z =
    ...
    (Math.sin(planet.longitude_ascending) *
      ...
      Math.cos(planet.longitude_ascending) *
        Math.sin(planet.true_anamoly() + planet.argument_periapsis)) *
    Math.cos(planet.orbital_inclination);
  console.log("after", group.position);
}
